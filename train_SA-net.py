...
tf.app.flags.DEFINE_string('key', 'stochastic_atrous_01',
...
if __name__ == "__main__":
    ...
    x = tf.cond(is_training,lambda: image_training,lambda: image_validation)[:,:,:,4:7] # Retrieving RGB Only features
    labels = tf.cond(is_training, lambda:annotation_training, lambda:annotation_validation)
    
    
    x = (x -  TF_IMG_MEAN[4:7] ) /  TF_IMG_STD[4:7] # Normalization
    
    pred, logits = sa_net.inference(x, is_training = is_training, num_classes = num_classes, num_blocks = [3,4,23,3])
    ...
    #Creating image output for visual analytics.
