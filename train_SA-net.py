...
import numpy as np
import stochastic_atrous_resnet_fcn
import time
...
from utils import create_border,  decode_labels, inv_preprocess, prepare_label, densecrf_postprocess, loss_IoU, create_reset_metric, isGPUAvailable
...
_KEY = 'stochastic_atrous_resnet_fcn'



IMG_MEAN = np.array((0,0,0), dtype=np.float32)
PATH = "./"
CHECKPOINT_FN = PATH + "deeplab_resnet.ckpt"
EPOCH_INIT = 0 
...
if __name__ == "__main__":
    ...
    x = tf.cond(is_training,lambda: image_training,lambda: image_validation)[:,:,:,4:7]
    labels = tf.cond(is_training, lambda:annotation_training, lambda:annotation_validation)
    
    
    x = (x -  TF_IMG_MEAN[4:7] ) /  TF_IMG_STD[4:7]
    
    
    #x = tf.placeholder(tf.float32, [batch_size, 224, 224, 3])
    #labels = tf.placeholder(tf.int32, [batch_size, 224, 224])

    pred, logits = stochastic_atrous_resnet_fcn.inference(x, is_training = is_training, num_classes = num_classes, num_blocks = [3,4,23,3])
    ...
    Images_Summary = tf.py_func(inv_preprocess, [ ( (x*TF_IMG_STD[4:7]) + TF_IMG_MEAN[4:7] ) /255 , BATCH_SIZE, IMG_MEAN, [0,1,2]], tf.uint8)
