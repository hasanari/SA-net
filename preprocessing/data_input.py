...
BASE_DATA_FOLDER = 'dataset/'


BASE_FOLDER = '../ssd1TB/big-file/temp/'

IDS_SLICER = '../ssd1TB/big-file/119/ids_slicer.npz'

DATA_READY_DIRECTORY = '../bigdata/hag_data_numpy/'
LABEL_READY_DIRECTORY = '../ssd1TB/big-file/119/numpy-label/'
...
def mapplabel(_label):
    ...
    ret = np.zeros(_label.shape ) #, dtype=np.int32)
...
def get_data_numpy(filenames, data_dir, _type):
        ...
        for id_ in tqdm(filenames):
            ...
            if  data_dir == DATA_READY_DIRECTORY:
                ...
                arr = np.zeros([ __temp.shape[0],  __temp.shape[1], 3 ], dtype=np.float32)
...
def write_to_disk_direct(log_file, basename, is_label, data, settype='train'):
    ...
    for i in (range(len(data))):
        ...
        if is_label:
            filename = data_dir +prefix+ '-'+ str(i)+'.png'
            scipy.misc.imsave( filename , data[i])
            #np.save( filename , data[i] )
        else:
            filename = data_dir +prefix+ '-'+ str(i)+'.png'
            scipy.misc.toimage(data[i]).save(filename)
...
def read_data_sets(train_dir, fake_data=False, one_hot=False):
    ...
    split_idx = np.load( IDS_SLICER  )['arr_0'].item()
    ...
    test_images =( get_data_numpy(split_idx['test'][0:], DATA_READY_DIRECTORY, 'val') )
    test_labels = ( get_data_numpy(split_idx['test'][0:], LABEL_READY_DIRECTORY, 'val') )

    
    write_log_to_disk(test_images,test_labels,'val')
